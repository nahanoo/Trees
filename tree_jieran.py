...
class Tree:
    ...
    def tree_loglikelihood(self, Q, use_P_cache =False):

        #clear any cached seq_matrix/log_scaled before run
        for node in self.nodes.values():
            node.seq_matrix = None
            node.log_scales = None
            node._P_cache = None
        if use_P_cache:
            for node in self.nodes.values():
                if node.branch_length is not None:
                    node._P_cache = expm(Q * float(node.branch_length)).astype(np.float32, copy=False)
